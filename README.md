...
This job fails if the linter doesn't pass, i.e. if there are any style issues in the code. The job prints out the style errors so that they're easier to notice:
![lint CI job error](screenshots/lint-ci-job-error.png)

### apidoc CI job

This job creates a browsable API documentation. For now the documentation can just be downloaded as a zip file:
![apidoc CI job artifact](screenshots/apidoc-ci-job-artifact.png)

### setup-node-build action
